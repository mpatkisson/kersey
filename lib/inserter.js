...
function inserter(options) {
    ...
    _that.saved = 0;
    _that.inserted = _options.inserted;
    ...
    _that.completed = function (err, completed) {
        if (_options.completed) {
            _options.completed(err, completed);
        }
    };
    ...
    _that.queue = function (record, isLast, callback) {
        ...
        if (_records.length === _that.threshold || isLast) {
            _mongoCollection.insert(_records, {w: 1}, function (err, result) {
                ...
                if (_that.inserted) {
                    _that.inserted(_that.saved, isLast);
                }
                if (_that.saved === _that.records) {
                    _that.completed(null, _that.saved);
                }
            });
            _records = [];
        }
